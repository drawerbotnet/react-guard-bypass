...
def resolve_cookie(html):
    ...
    if not keys:
        raise ValueError('NE OBOSHLO PIZDAA')
...
#Создаем сессию (необяз, можете напрямую get, post слать если не используете другие куки сайтаб для удобства)
s=requests.Session()

#Получаем хекс лист для обхода защиты
html=s.get(
"https://gta-trinity.com/",
...
#Обходим пердеж react.su и получаем нужную нам страницу
html=s.get(
"https://gta-trinity.com/",
headers={"user-agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36"}
).text
